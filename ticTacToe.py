...
class Player(AgentI):
    def __init__(self, name, prob, biddingStrategy, symbol, totalChips, exp_rate=0.6):
        self.name = name
            ...
            if np.random.uniform(0, 1) <= self.exp_rate:
                ...
                for b in range(int(availableTokens) + 1):
                    if (state.tieBreaker==self.symbol):  #have tiebreaker
                        value = 0 if self.states_value.get(stateHash, b) is None else self.states_value.get(stateHash, b)
                        if prob:
                            bidWinProb = self.getProb(state.getHash(), b)
                            value *= bidWinProb
                        if value >= value_max:
...
if __name__ == "__main__":
    ...
    AverageError(chips, prob, rlStrat, 2, 20000)
